...
//			Ticker.tick(5000,test,1);
		...
		private function initLsn():void
		{
			...
			event.listen(SEvents.GIFT_CAC_ANIMATION, addGiftAmt);
		...
		{
			var event:IEventManager = Context.getContext(CEnum.EVENT) as IEventManager;
			event.remove(SEvents.GIFT_CAC_ANIMATION, addGiftAmt);
...
//		}
		...
		private function addGiftAmt(obj:Object):void
		{
			...
			if (obj.giftSwfPath && obj.giftSwfPath != "")
			{
				...
				while (count > 0)
				{
					var at:AnimationObject = AnimationObject.getAmd(obj.giftSwfPath, AnimationType.ATYPE_CAC, _layer);
					at.data = obj;
					arr[arr.length] = at;
					count--;
				}
...
//				addAmdData(arr);
			...
			{
				var at1:AnimationObject = AnimationObject.getAmd(obj.giftSwfPath1, AnimationType.ATYPE_CAC_LINE, _lineLayer);
				at1.data = obj;
				addLineData(at1);
			}
		 ...
		 *
		 */
		public function addLineData(data:AnimationObject):void
		{
