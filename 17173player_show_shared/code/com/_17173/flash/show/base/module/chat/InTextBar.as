package com._17173.flash.show.base.module.chat
{
	...
	public class InTextBar extends Sprite
	{
		...
		private var smileFace:SmileFace;
		...
		private function addChildren():void
		{
			...
			var tf:TextFormat = new TextFormat(FontUtil.f, 12, 0x9774C6);
			tf.leftMargin = 5;
			...
			smileFace.mouseChildren = false;
			smileFace.buttonMode = true;
			smileFace.y = (height - smileFace.height) * .5 + 2;
			smileFace.x = inTxt.width + 5;
			this.addChild(smileFace);
		...
		public function set backgroundColor(_color:Number):void
		{
			...
			this.graphics.drawRect(0, 0, inTxt.width + smileFace.width + 5, 26);
		...
		{
			var tar:SmileFace = event.target as SmileFace;
			if(!tar)
			{
				return;
			}
			...
			if(_e)
			{
				var globPos:Point = this.localToGlobal(new Point(0, tar.y));
				_e.send(SEvents.OPEN_SMILE_PANEL, {"content":this, x:globPos.x, y:globPos.y - 200});
			}
		}
