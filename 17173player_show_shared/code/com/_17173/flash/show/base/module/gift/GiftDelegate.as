package com._17173.flash.show.base.module.gift
{
	...
	public class GiftDelegate extends BaseModuleDelegate
	{
		...
		{
			super();
			_s.socket.listen(SEnum.R_GIFT_MSG.action, SEnum.R_GIFT_MSG.type, onGiftCache);
...
//			Debugger.log(Debugger.INFO, "[gift]", "礼物开关状态: " + showData.showGift);
		...
		private function onActGiftMessage(data:Object):void{
			...
			if(data.ct.activity == 0){
				data.ct.giftType = AnimationType.ATYPE_TUZI;
			}else if(data.ct.activity == 1){
				data.ct.giftType = AnimationType.ATYPE_TECH;
			}
		...
		private function sentToModule(data:Object,isSendChat:Boolean = true):void{
			try{
				...
				if(giftClose){
					if(vo.giftType == GiftData.GIFT_TYPE_FLOWER){
				...
				if (vo.giftType == GiftData.GIFT_TYPE_NORMAL_ANM)
				{
					_e.send(SEvents.GIFT_CAC_ANIMATION, vo);
				}
				else
				{
					var type:String = vo.giftType;
		...
		private function checkSendToBigAnm(type:String):Boolean{
			...
			//车类动画，花类动画//各种活动
			if(type == GiftData.GIFT_TYPE_CAR || type == GiftData.GIFT_TYPE_FLOWER  
		...
		{
			Debugger.log(Debugger.ERROR, "[init]", "礼物数据加载错误!", data.msg);
		}
	}
}