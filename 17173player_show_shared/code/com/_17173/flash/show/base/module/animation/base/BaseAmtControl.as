package com._17173.flash.show.base.module.animation.base
{
	...
	public class BaseAmtControl
	{
		 ...
		 *当前播放数据 
		 */		
		protected var _cPlayData:AnimationObject = null;
		/**
		 *是否播放子动画 
		 */		
		...
		protected function playNewForLoad():void{
			if(_datas.length > 0){
				_cPlayData = _datas.shift() as AnimationObject;
		 ...
		 * 
		 */		
		public function addData(data:AnimationObject):void{
			_datas.push(data);
			loadNext();
		}
		/**
		...
		protected function loadNext():void{
			if(_playingItem){
				...
				//播放下一个
				var data:AnimationObject = _datas[0] as AnimationObject;
				if(data.loaded){
					playNewForLoad();
				}else{
					//判断如果有缓存的，则不用新建的对象;
//					var tData:AnimationObject = AnimationObject.getAmdByPath(data.url);
//					if(tData){
//						data = null;
//						_datas[0]  = null;
//						_datas[0] = tData;
