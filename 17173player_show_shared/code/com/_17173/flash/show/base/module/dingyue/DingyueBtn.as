package com._17173.flash.show.base.module.dingyue
{
	...
	public class DingyueBtn extends Button
	{
		...
		public function DingyueBtn(showCount:Boolean = true, showAlert:Boolean = true)
		{
			...
			var tfm:TextFormat = new TextFormat("Microsoft YaHei,微软雅黑,宋体",12,0xffffff,true);
		...
		private function addLsn():void{
			((Context.getContext(CEnum.SERVICE) as IServiceProvider)).socket.listen(SEnum.UPDATE_DINGYUE.action,SEnum.UPDATE_DINGYUE.type,updateCount);
		}
		...
		private function sendShowGuide():void{
			...
			point.x += width/2;
			if(_showCount){
				(Context.getContext(CEnum.EVENT) as EventManager).send(DINGYUE_GUIDE,point);
			}
		}
		...
		override protected function onResize(e:Event=null):void{
			...
			point.x += width/2;
			if(_showCount){
				(Context.getContext(CEnum.EVENT) as EventManager).send(DINGYUE_GUIDE_RP,point);
			}
		}
