package com._17173.flash.player.business.file
{
	...
	public class FileVideoData extends VideoData
	{
...
//					}
		...
		/**
		 * 获取调度信息中是否有比当前清晰度更低的清晰度
		 * @param target 当前清晰度
		 * @return 比目标清晰度更小的清晰度，-1为没有
		 * 
		 */		
		public function getPreviousDef(target:String):String {
			var re:String = "-1";
			for (var key:String in _info) {
				if (int(key) < int(target) && int(key) > int(re)) {
					re = key;
				}
			}
			return re;
