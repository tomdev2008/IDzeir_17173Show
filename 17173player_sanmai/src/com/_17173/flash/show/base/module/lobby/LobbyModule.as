package com._17173.flash.show.base.module.lobby
{
	...
	public class LobbyModule extends BaseModule
	{
		...
		public function LobbyModule()
		{
			...
			_vTxt.htmlText = "<font size='24' color='#ffffff'>"+_local.get("vTitle","lobby")+"</font>";
			_box.addChild(_vTxt);
			...
			_vMoreTxt.mouseEnabled = _gMoreTxt.mouseEnabled = true;
			_vMoreTxt.htmlText = _gMoreTxt.htmlText = "<a href='event:myText'><font size='14' color='#ffffff'>"+_local.get("more","lobby")+"</font></a>";
		...
		public function show():void
		{
			...
			this.x = -this.width;
			this.visible = true;
			TweenMax.to(this, 1, {x: 0, onComplete:onComplete,onCompleteParams:[true]});
		}
		...
		private function update():void
		{
			...
			var rect:Rectangle = (Context.getContext(CEnum.UI) as IUIManager).sceneRect;
			_box.x = rect.left + (rect.width - _box.width) >> 1;
			_box.y = Math.max(rect.top + 20, rect.top + (rect.height + stage.stageHeight - rect.bottom - _box.height) >> 1);
			_bglayer.x = rect.left;
			_bglayer.y = rect.top;
			_bglayer.width = rect.width;
			_bglayer.height = rect.height + stage.stageHeight - rect.bottom;
			closeBut.x = rect.width - closeBut.width - 10;
			closeBut.y = rect.top + 10;
			
			_vMoreTxt.x = _box.x + _box.width - _vMoreTxt.width - 5;
			_vMoreTxt.y = _box.y + 15;
