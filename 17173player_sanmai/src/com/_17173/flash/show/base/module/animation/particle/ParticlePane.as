package com._17173.flash.show.base.module.animation.particle
{
	...
	import flash.geom.Point;
		...
		public class ParticlePane extends Sprite
		{
			public function ParticlePane(tw:int = 1500,th:int = 600,singC:Boolean = false)
			{
				displayWidth = tw;
				displayHeight = th;
				super();
...
//			this.addChild(frame);
		...
		private function onEnter(evt:Event):void {		
			...
			if (cCount > stCount) {
				...
				count = 0;
				flareOriginX = 125 + Math.random()*((displayWidth-300));
				flareOriginY = 90 + Math.random()*120;
				var i:int;
				...
				for (i = 0; i < numFlares; i++) {
					...
					thisParticle.airResistanceFactor = 0.010;
					//envelope - this determines how long a flare will live.
					thisParticle.envelopeTime1 = 10 + 35*Math.random();
			...
			while (particle != null) {
				...
				particle.vel.x += 0.2*(Math.random()*2 - 1);
				particle.vel.y += 0.2*(Math.random()*2 - 1) + gravity;
				particle.vel.z += 0.2*(Math.random()*2 - 1);
			...
			while (spark != null) {
				...
				spark.vel.x += 0.2*(Math.random()*2 - 1);
				spark.vel.y += 0.2*(Math.random()*2 - 1) + gravity;
