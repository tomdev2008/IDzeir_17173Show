...
//			ui.registerTip(_exchangeBtn,lc.get("duihuan_tip","bottom"))
		...
		private function loadIcon():void{
			...
			var sType:String = uint(user.starLevel)>25?".swf":".png";
			var rType:String = uint(user.richLevel)>25?".swf":".png";
			
			var path:String = "assets/img/level/cflv"+user.richLevel+rType;
			if(_richLvl && _richLvl.parent){
				_richLvl.parent.removeChild(_richLvl);
			}
			_richLvl ||= new BitmapMovieClip(true,24,24);
			_richLvl.url = path;
			_richLvl.x = 92;
			_richLvl.y = 65;
			...
			path = "assets/img/level/lv"+user.starLevel+sType
			if(_starLvl && _starLvl.parent){
				_starLvl.parent.removeChild(_starLvl);
			}
			_starLvl ||= new BitmapMovieClip(true,24,24);
			_starLvl.url = path;
			_starLvl.x = 92;
			_starLvl.y = 100;
