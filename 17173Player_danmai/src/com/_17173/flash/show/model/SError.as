package com._17173.flash.show.model 
{
	...
	public class SError
	{
		...
		private const DICT:Object = {
			...
			///////////////////
			//用户已经登陆过该房间
			"-000001":{"okBack":onRefreshPageOut, "cancelBack":onRefreshPageOut, "msg":{"key":"409004", "pack":"error"}}, 
			...
			//飞屏超过20个字符
			"000014":{"handler":showAlert,"msg":{"key":"000014","pack":"error"}},
			//钱不够
			...
			"409004":{"okBack":kickGoHome, "destroy":true,"cancelBack":kickGoHome, "msg":{"key":"409004", "pack":"error"}}, 
		...
		public function SError(id:String, msg:String,data:Object = null)
		{
			_id = id;
			_msg = msg;
			_data = data;
		...
		private function showAlert(data:Object = null):void {
			...
			//系统错误消息，销毁socket连接
			if(data && data.hasOwnProperty("destroy")&&data["destroy"])
			{
		...
		private function chatError(data:Object=null):void{
			...
			//msg为发送失败的内容
			var d:Object = {};
			d.chatMsg = data["chatMsg"];
			if(data.hasOwnProperty("msg"))
			{
				d.result = getString(data.msg);
			}
			_e.send(SEvents.CHAT_NOTIFY,d);
		}
		 ...
		 * 并且断开连接 
		 */		
		private function onRefreshPageOut():void {
			var s:IServiceProvider = Context.getContext(CEnum.SERVICE) as IServiceProvider;
			s.socket.close();
			Context.variables["isKick"] = 1;
			Util.refreshPage();
		}
		...
		private function kickGoHome():void{
			Context.variables["isKick"] = 1;
			Util.toUrl(SEnum.home, "_self");
		}
		 ...
		 * 弹出登陆面板 
		 */		
		private static function toLogin(value:* = null):void{
		...
		public static function handleServerError(error:Object,showAlert:Boolean = true):void {
			...
			var se:SError = new SError(error.code, error.msg,error["ct"]);
			se.handle();
