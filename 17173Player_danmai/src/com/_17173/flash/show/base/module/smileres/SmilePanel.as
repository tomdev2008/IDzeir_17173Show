package com._17173.flash.show.base.module.smileres
{
	...
	public class SmilePanel extends BaseModule
	{
		...
		{
			this.graphics.lineStyle(1, 0xAF0074, .5);
			this.graphics.beginFill(0x490056);
		...
		private function fileSmileres():void
		{
			...
			totalPage = Math.ceil(gp.length / (ROW * COL));
			text.text = this.curPage + "/" + this.totalPage;
			this.bottomContainer.visible = this.totalPage > 1;
		...
		private function getForm(tar:Object):Object
		{
			var obj:Object = {x: tar.x, y: tar.y};
			switch (tar.dir)
			{
				case 1:
					obj.x -= ((this.width) >> 1) + 100;
					obj.y -= (this.height);
					break;
				case 2:
					obj.x -= ((this.width) >> 1) + 80;
					obj.y += (0);
					break;
				case 3:
					break;
		...
		private function getTo(tar:Object):Object
		{
			var obj:Object = {x: tar.x, y: tar.y};
			switch (tar.dir)
			{
			...
			obj.onCompleteParams = [tar.content];
			obj.alpha = 1;
			return obj;
		}
		...
		public function show(target:Object = null):void
		{
			...
			TweenMax.fromTo(this, .5, getForm(target), getTo(target));
...
class SmileBox extends Sprite
{
	...
	public function SmileBox(size:Number)
	{
		...
		this.graphics.beginFill(0x31003E);
