package com._17173.flash.show.base.module.chat.vo
{
	...
	public class UserEntryExitVo extends BaseChatVo
	{
		...
		override protected function initVo():void
		{
			...
			if (!_user.hidden)
			{
				var has:Boolean = this.createGraphicFromArray(this.getUsericon(_user));
				textElement = new TextElement((has?" ":"")+_user.name, tf);
			...
			{
				textElement = new TextElement(_ilocal.get("msg_hidenEntry", "chatPanel"), FontUtil.getFormat(0xEC1967));
			}
			this._elems.push(textElement);
		}
