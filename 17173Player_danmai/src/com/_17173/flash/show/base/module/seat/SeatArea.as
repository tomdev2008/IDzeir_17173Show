package com._17173.flash.show.base.module.seat
{
	...
	public class SeatArea extends BaseModule
	{
		...
		public function SeatArea()
		{
			...
			this.graphics.drawRect(0, 0, 843, 195);
...
//			event.listen(SEvents.NORMAL_GIFT_MESSAGE,addMessage);
		...
		public function addMessage(msg:Object = null):void{
		...
		private function seatStand(seat:Seat):void{
			...
			var msg:Object = _needShowMsg.shift();
			seat.showMessageAndEff(msg);
			
			/** 转发座位坐标，给舞台展示效果 **/
			if(msg.effectType == AnimationType.GIFT_TYPE_NORMAL)
			{
				var event:EventManager = Context.getContext(CEnum.EVENT) as EventManager;
				//var point:Point = seat.parent.globalToLocal(seat.localToGlobal(new Point(0, 0)));
				var point:Point = seat.localToGlobal(new Point(0, 0));			
				
				event.send(SEvents.STAGE_GIFT_SEAT_COOR,{"msg":msg,"point":point});
			}
		}
		/**
		 *站立结束  
