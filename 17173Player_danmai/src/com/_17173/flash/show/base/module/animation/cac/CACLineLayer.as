package com._17173.flash.show.base.module.animation.cac
{
	...
	public class CACLineLayer extends BaseAnimationLayer
	{
		...
		public function addMsg(obj:IAnimationPlay):void{
		...
		private function onLoad():void{
			...
			for (var i:int = 0; i < len; i++) 
			{
				...
				if(hv.canPlayNext){
					...
					var tanm:IAnimationPlay;
					for (var j:int = 0; j < len1; j++) 
					{
						var anm:IAnimationPlay = _msgs[j];
						if(anm.loaded){
							tanm = anm;
							_msgs.splice(j,1);
							break;
					...
					if(tanm){
						var bmp:Bitmap = (tanm as IAnimationPlay).getBmp();
						hv.playItem(bmp);
//						hv.playItem(tanm.mc);
		...
		private function onPlayEnd(e:MoveEvent):void{
			...
			for (var i:int = 0; i < len; i++) 
			{
				var anm:IAnimationPlay = _plays[i];
				if((anm as IAnimationPlay).bmp === obj){
