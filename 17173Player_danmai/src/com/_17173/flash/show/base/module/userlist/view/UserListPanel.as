package com._17173.flash.show.base.module.userlist.view
{
	...
	[Event(name="touchBottom", type="com._17173.flash.show.base.module.userlist.view.ListEvent")]
	/**
	 * 单个用户列表显示
	 * @author idzeir
	...
	public class UserListPanel extends VGroup
	{
		...
		private var _brokerBox:BrokerBox;
		
		private var _guestBox:GuestBox;

		private var _showGuest:Boolean = false;
		...
		public function UserListPanel(itemRender:Class = null)
		{
			...
			_guestBox = new GuestBox();
			_guestBox.width = userList.width;
			_guestBox.visible = false;
			this.addChild(this._guestBox);
		}
		...
		protected function onMaxScroll(event:ProgressEvent):void
		{
			...
			{
				if(this.hasEventListener(ListEvent.TOUCH_BOTTOM))
				{
					//滚动条到达底部触发事件
					this.dispatchEvent(new ListEvent(ListEvent.TOUCH_BOTTOM));
				}
			}
		}
		...
		public function addItem(data:Object):DisplayObject
		{
			...
			if(user.broker)
			{
				if(!_brokerBox)
				{
					_brokerBox = new BrokerBox();
				}
				_brokerBox.user = user;
				this.addChildAt(_brokerBox,0);
				//刷新用户列表
				resize();
				return _brokerBox;
			}
			...
			if(user&&indexOf(user.id)>=0)
			{
				...
				if(tag!="mic")
				{
					...
					if(showData.type == "0")
					{
						...
						if(ownerID!=user.id)
						{
							userList.updateItem("id",user.id,user);
							userList.relist();
						}
					}else{
						userList.updateItem("id",user.id,user);
		...
		public function removeItem(uid:int):void
		{
			...
			var index:int = indexOf(String(uid))
			if(index == -1)
			{
				return;
			}
			
			var user:IUserData = dataArr.splice(index, 1)[0] as IUserData;
			if(user&&user.broker)
			{
				if(this._brokerBox&&this.contains(this._brokerBox))
				{
					this.removeChild(this._brokerBox);
					resize();
				}
				//刷新用户列表
				return;
		 ...
		 * 
		 */		
		public function set showGuest(bool:Boolean):void
		{
			_showGuest = bool;
			this._guestBox.visible = bool;
			if(!bool)
			{
				if(this.contains(this._guestBox))
				{
					this.removeChild(this._guestBox);
				}
			}else{
				this.addChild(this._guestBox);
			}
		...
		public function resize():void
		{
			if(_sh==0)return;
			var uh:int = _sh - 33;
			if(this._brokerBox&&this.contains(this._brokerBox)&&this._brokerBox.user)
			{
				uh -= 34 + 2;
			}
			if(this._showGuest)
			{
				uh -= this._guestBox.height + 2;
			}
			userList.height = uh + 2;
			this.rePos();
		}
		...
		override public function set height(value:Number):void{
			_sh = value;
			var theight:int = value - 33 - (this._showGuest?this._guestBox.height:0);
			if(this._brokerBox&&this._brokerBox.user)
			{
				theight -= 34;
			}
			//最小高度
			userList.height = theight;
			this.rePos();
		}
	}
}