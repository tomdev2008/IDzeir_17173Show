package com._17173.flash.show.base.module.video.base.live
{
	...
	public class LiveVideoManager extends BaseVideoManager
	{
		...
		override protected function onVideoStart():void {
			...
			onStartQM();
			//保存拉流stream
			StreamMessage.stream = this.source.stream;
		}
		...
		private function onFullTime():void
		{
			if (_bufferCTime != 0)
			{
				...
				var streamName:String = vd.name;
				var mstId:String = (Context.getContext(CEnum.USER) as IUser).me.id
				...
				var bstr:String = "{}";
				try{
					bstr = "{" + _bufferCTime + "," + cid + "," + cdn + "," + opt + "," + streamName + "," + mstId +","+ showData.ownerID +","+showData.liveInfo.masterNo + "}";
				}catch(e:Error){
					Debugger.log(Debugger.INFO,"[QM]","统计输出错误",e.message);
				}
				if(_bufferTimeString == ""){
