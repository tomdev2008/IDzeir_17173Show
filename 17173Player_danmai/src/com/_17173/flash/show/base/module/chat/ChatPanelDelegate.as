package com._17173.flash.show.base.module.chat
{
	...
	public class ChatPanelDelegate extends BaseModuleDelegate
	{
		...
		private function addListeners():void
		{
			...
			//飞屏控件数据
			this._e.listen(SEvents.FLY_SCREEN_DATA,onFlyScreenData);
			//大小烟花消息
		...
		override protected function clear():void
		{
			...
			//飞屏控件数据
			this._e.remove(SEvents.FLY_SCREEN_DATA,onFlyScreenData);
			//大小烟花消息
		 ...
		 * 
		 */		
		private function onFlyScreenData(value:*):void
		{
			if(!this.validate({_handle:onFlyScreenData, data:value}))
			{
				return;
			}	
			this.module.data = {"onFlyScreenData":[value]};
		}
		...
		private function onClearMsg(value:* = null):void
		{
			...
			this.module.data = {"clearHistoryMsg":[value]}
		}
		/**
		 * 历史聊天对象
		 * @param data
...
//				if((Context.getContext(CEnum.USER) as IUser).me.id == value["userInfo"].userId)
//					_e.send(SEvents.TASK_CHAT_SUCCESS);
			}
