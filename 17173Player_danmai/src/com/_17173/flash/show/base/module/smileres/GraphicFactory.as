	...
	public class GraphicFactory implements IGraphic
	{
		...
		public function createGraphic(faces:Array,_factory:Function):void
		{
			...
			do
			{
				var data:Object = {};
				data.frame = total;
				if(faces[total].hasOwnProperty("width"))
				{
					data.width = faces[total].width;
				}else{
					data.width = 24;
				}
				if(faces[total].hasOwnProperty("height"))
				{
					data.height = faces[total].height;
				}else{
					data.height = 24;
				}
		...
		public function getElements(msg:String, elf:ElementFormat = null):Vector.<ContentElement>
		{
			...
			while(graphicArr.length > 0)
			{
				...
				if(graphic)
				{
					var graphicItem:BitmapMovieClip;

					if(!graphic.hasOwnProperty("injected"))
					{
						graphicItem = new BitmapMovieClip(true,graphic.width,graphic.height);//this._factory.apply(null,[graphic.url]);
						graphicItem.url = graphic.url;
					}
					else
					{
