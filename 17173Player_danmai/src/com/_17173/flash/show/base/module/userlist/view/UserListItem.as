package com._17173.flash.show.base.module.userlist.view
{
	...
	public class UserListItem extends Sprite implements IItemRender
	{
		...
		private var _masterNo:MasterNoMovie;
		...
		public function UserListItem()
		{
			...
			_masterNo = new MasterNoMovie(true,63,17);
		...
		public function startUp(value:Object):void
		{
			...
			{
				_box.addChild(_masterNo);
				_masterNo.url = user.masterNoSwf;
				_masterNo.no = user.masterNo;
			}			
			...
			if(user.broker)
			{
				...
				//非主播显示
				//是否包含icon图标
				var has:Boolean = user.vip||user.isAdmin||user.agent||user.normalAdmin||user.saler;
				//vip
				if(user.vip>0)
				{
